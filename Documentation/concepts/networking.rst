...
As the IPv4 addresses used for pods are typically allocated from RFC1918
private address blocks and are thus not publicly routable. Cilium will
automatically masquerade the source IP address of all traffic that is leaving
the cluster to the IPv4 address of the node as the node's IP address is already
...
Setting the masquerading interface
  ...
  option ``egress-masquerade-interfaces: eth+`` can be used.
